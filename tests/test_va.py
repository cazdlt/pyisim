...
def test_get_property_value(sesion):    
    ...
    one=update_property.get_property_value(sesion,property_file,"ITIM.java.access.util")
...
def test_add_property(sesion):
    ...
    property_name="test"
    property_value="test"
    r=update_property.add_property(sesion,property_file,property_name,property_value)
...
def test_update_property(sesion):
    ...
    old_val=update_property.get_property_value(sesion,property_file,property_name)["PropertyValue"]
    if property_value==old_val:
        property_value+="X"

    r=update_property.update_property(sesion,property_file,property_name,property_value)
    assert r=="OK"

    new_val=update_property.get_property_value(sesion,property_file,property_name)["PropertyValue"]
